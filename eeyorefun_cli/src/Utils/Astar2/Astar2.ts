class AStar2 {
	...
	public search(): boolean {
		...
		while (!this.isEnd) {
			...
			var _tempList: Point[] = [];
			for (var a: number = startX; a < endX; a++) {
				_tempList.push(new Point(a, node.y));
			}
			for (var b: number = startY; b < endY; b++) {
				_tempList.push(new Point(node.x, b));
			}
			////一次九宫格循环节点
			// for (var n in _tempList) {
			for (var i: number = startX; i <= endX; i++) {
				for (var j: number = startY; j <= endY; j++) {

					////当前要被探查的节点
					// var testNode: NodePoint = this._grid.getNode(_tempList[n].x, _tempList[n].y);
					var testNode: NodePoint = this._grid.getNode(i, j);

					////若当前节点等于起始节点 或 不可通过 或 当前节点位于斜方向时其相邻的拐角节点不可通过
					if (testNode == node || !testNode.walkable || !this._grid.getNode(node.x, testNode.y).walkable || !this._grid.getNode(testNode.x, node.y).walkable) {
						continue;
					}
					...
					var g: number = node.g + cost * testNode.costMultiplier;
					var h: number = this._heuristic(testNode);
					var f: number = g + h;

					////在开启列表或关闭列表中找到 表示已经被探测过的节点
					if (this._open.indexOf(testNode) != -1 || this._closed.indexOf(testNode) != -1) {
	...
	private manhattan2(node: NodePoint): Number {
